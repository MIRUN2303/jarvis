...
class VoiceAuthenticator:
    def __init__(self, reference_file="reference_voice.wav", threshold=0.25):
        ...
        if self.enabled:
            print("[VoiceAuth] Initializing Speaker Verification model (this may take a moment)...")
            # Uses ECAPA-TDNN model trained on VoxCeleb
    ...
    def verify(self, audio_data: np.ndarray) -> bool:
        ...
        """
        if not self.enabled:
            return True # Fallback to accepting all audio if not setup
