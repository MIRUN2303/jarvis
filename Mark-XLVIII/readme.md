...
| 🔔 Proactive Check-ins | After 15 minutes of silence ROOKI checks context and offers something genuinely useful — no hardcoded rules, Gemini decides |
...
The startup briefing now sends Phase 2 (news fetch) while Phase 1 audio (the greeting) is still playing. Previously, Phase 2 waited for Phase 1 to fully complete. The 1.5-second overlap means the news headline is ready by the time ROOKI finishes saying "Good morning."
...
Screen capture is guarded against echo loops. If ROOKI speaks about the screen and the microphone picks up its own voice, a secondary `screen_process` call can be triggered. Mark XLVIII blocks duplicate calls with a 4-second cooldown and a `_vision_busy` flag. Both are fully reset when a new session connects — fixing a bug where the cooldown would persist across reconnects and block legitimate requests.

### 🌐 Language-Aware Address — Never Mixed
The system prompt now enforces: **Turkish → efendim**, **English → sir**, never mixed. Mark XLVII's prompt said "Always call sir to user" which caused ROOKI to say "sir" mid-Turkish sentence. Fixed.
...
All transient vision and interrupt flags (`_pending_vision`, `_vision_busy`, `_vision_cam_active`, `_vision_close_pending`, `_interrupted`) are fully reset whenever a new Gemini session connects. Previously, state from a crashed session could carry over and leave ROOKI in a broken state until restart.
...
├── core/
│   └── prompt.txt           # ROOKI personality and tool-routing rules
└── config/
...
Engineered by a developer building a real-world ROOKI-style assistant.
