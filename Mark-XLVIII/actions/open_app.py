...
def _launch_windows(app_name: str) -> bool:
    ...
    # ── PATH search: find binary & launch appropriately ──
    _binary = shutil.which(app_name) or shutil.which(app_name.split(".")[0])
    if _binary:
        try:
            # Check if it's a GUI app or console/CLI app by PE subsystem
            _is_gui = False
            try:
                import struct
                with open(_binary, 'rb') as _f:
                    _f.seek(0x3c)
                    _pe_off = struct.unpack('<H', _f.read(2))[0]
                    _f.seek(_pe_off + 0x5c)
                    _subsys = struct.unpack('<H', _f.read(2))[0]
                    _is_gui = (_subsys == 2)  # IMAGE_SUBSYSTEM_WINDOWS_GUI
            except Exception:
                pass

            if _is_gui:
                # GUI app → launch directly
                subprocess.Popen([_binary])
            else:
                # CLI / console app → open cmd window and run the command
                subprocess.Popen(
                    ["cmd", "/k", app_name],
    ...
    # ── Try as a URI scheme or direct command ──
    if ":" in app_name:
        try:
