...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".rooki_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".rooki_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ROOKI profile")
                rooki_path = str(Path.home() / ".rooki_profiles" / "firefox_rooki")
                Path(rooki_path).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(rooki_path, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".rooki_profiles" / "safari")
        ...
        rooki_profile = str(Path.home() / ".rooki_profiles" / self.browser_name)
        Path(rooki_profile).mkdir(parents=True, exist_ok=True)
        _safe_print(f"[Browser] Retrying with ROOKI profile: {rooki_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(rooki_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            _safe_print(f"[Browser] OK Launched [{label}] with ROOKI profile")
    ...
    async def close_tab_by_index(self, index: int) -> str:
        page = await self._get_page()
        ctx  = page.context
        pages = ctx.pages
        if index < 0 or index >= len(pages):
            return f"Invalid tab index {index}. Open tabs: {len(pages)}"
        target = pages[index]
        try:
            await target.close()
        except Exception as e:
            return f"Could not close tab [{index}]: {e}"
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "rooki_screenshot.png")
...
def browser_control(
    ...
    try:
        if action == "go_to":
            ...
            if idx is not None:
                result = sess.run(sess.close_tab_by_index(int(idx)))
            else:
