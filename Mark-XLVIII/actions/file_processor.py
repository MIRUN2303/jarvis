"""
file_processor.py — ROOKI Universal File Processor
...
from core.openrouter_client import ask_llm, ask_llm_multimodal
...
def _gemini_client():
    class _W:
        def generate_content(self, contents, **kw):
            ...
            return type('R', (), {'text': ask_llm(prompt, max_tokens=256)})()
    return _W()
...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            result    = ask_llm_multimodal(prompt, images=[img_bytes], max_tokens=256)
            result    = result.strip()
        ...
        try:
            ...
            try:
                result = ask_llm_multimodal(
                    ...
                    images=[content], max_tokens=256,
                )
            except Exception:
                from google import genai
                _gemini_audio = genai.Client(api_key=_get_api_key())
                response = _gemini_audio.models.generate_content([
                    "Transcribe all speech in this audio file accurately.",
