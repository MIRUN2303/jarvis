...
def _get_gemini():
    class _W:
        def generate_content(self, contents, **kw):
            ...
            return type('R', (), {'text': ask_llm(prompt, max_tokens=384)})()
    return _W()
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"rooki_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"rooki_debug_{int(time.time())}.png"
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        analysis = ask_llm_multimodal(analysis_prompt, images=[image_bytes], max_tokens=384)
