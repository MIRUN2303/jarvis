"""
dashboard/server.py — ROOKI Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "ROOKI Uploads",
        Path.home() / "Documents" / "ROOKI Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'ROOKI-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"ROOKI Dashboard Port {port}"
        prog_rule  = "ROOKI Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="rooki_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until ROOKI is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "rooki.key").exists() and (certs / "rooki.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in ROOKI to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('rooki_token','{tok}');
  sessionStorage.setItem('rooki_key','{key}');
  localStorage.setItem('rooki_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to ROOKI…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "rooki.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "rooki.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in ROOKI UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
